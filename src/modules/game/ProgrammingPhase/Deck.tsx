...
/**
 * @author Sachin Baral 2025-09-29 19:51:47 +0200 13
 */
  ...
  if (isSmall) {
    ...
    return (
      <div className={`relative ${className}`}>
        <div className={`relative ${isSmall ? "w-12 h-16" : "w-24 h-32"} mx-auto`}>
          {/* Create a visual stack of cards */}
          {[...Array(Math.min(3, Math.max(1, remainingCards > 0 ? remainingCards : 1)))].map(
            (_, index) => (
              <Card
                key={index}
                className={`
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="text-center mb-4">
        ...
        <p className="text-sm text-cyan-400">{remainingCards} cards remaining</p>
      </div>

      {/* Visual Deck Stack */}
      <div className="relative w-24 h-32 mx-auto">
        {/* Create a visual stack of cards */}
        {[...Array(Math.min(5, Math.max(1, remainingCards)))].map((_, index) => (
          <Card
            key={index}
            className={`
              absolute w-full h-full
          ...
          >
            <div className="w-full h-full flex items-center justify-center">
              ...
              <div className="w-16 h-20 rounded-lg bg-gradient-to-br from-cyan-500/20 to-purple-500/20 border border-cyan-400/50 flex items-center justify-center">
                <div className="text-xs text-cyan-300 font-mono transform rotate-45">RR</div>
              </div>
            </div>
          </Card>
        ))}
